...
public class ItemStreamControllerTest {
	...
	public void setUp() {
		...
		itemReactiveCappedRepository.insert(itemCappedFlux).doOnNext((itemCapped -> {
	...
	public void testStreamAllItems() {
		...
		Flux<ItemCapped> itemCappedFlux = webTestClient.get().uri(ItemConstants.ITEM_STREAM_END_POINT_V1).exchange()
				.expectStatus().isOk().returnResult((ItemCapped.class)).getResponseBody().take(5);
